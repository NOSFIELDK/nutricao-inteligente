...
import { supplements } from "@/data/catalog";
...
export default function SupplementsPage() {
  ...
  const list: Recommendation[] = supplements.map((s) => ({
    item: s,
    score: 1,
    reasons: s.tags.slice(0, 3).map((t) => ({ tag: t, label: t })),
  }));
      ...
      <Modal open={!!selected} title="Adicionar ao plano" onClose={() => setSelected(null)}>
        {selected ? (
          <div className="grid gap-4">
            <div className="text-sm text-muted">{selected.type === "supplement" ? selected.name : "Item"}</div>
